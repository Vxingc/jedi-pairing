...
func main() {
	...
	{
		fmt.Printf("[*] Per-dimension bit widths:")
		for _d := 0; _d < NumDims; _d++ {
			fmt.Printf(" dim%d=%d", _d, DimBitLengths[_d])
		}
		fmt.Printf("  (total=%d)\n", TotalBitLength)
	}
	{
		fmt.Printf("[*] Parent-expand dims:")
		for _d := 0; _d < NumDims; _d++ {
			fmt.Printf(" dim%d=%v", _d, DimExpandAsParent[_d])
		}
		fmt.Println()
	}
	...
	// For dimensions beyond dim3, use full domain by default.
	// Override at query time with -extra-width if a narrower range is desired.
	for _d := 4; _d < NumDims; _d++ {
		_maxDom := int64(1<<DimBitLengths[_d]) - 1
		query.Bounds[_d] = [2]int64{0, _maxDom}
	}
