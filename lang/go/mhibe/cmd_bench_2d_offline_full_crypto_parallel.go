...
func materializeDatabasePointKeys(
	...
	sampleSlotByIndex := make(map[int]int, checks)
	for i := 0; i < checks; i++ {
		sampleAt[i] = i * len(points) / checks
		sampleSlotByIndex[sampleAt[i]] = i
	}

	sampled := make([]DerivedPatternKey, checks)
	...
	if err := parallelFor(len(points), func(idx int) error {
		...
		if slot, ok := sampleSlotByIndex[idx]; ok {
			sampled[slot] = DerivedPatternKey{
				Pattern: pattern,
				Attrs:   attrs,
				Key:     key,
			}
	...
	return sampled, int(totalBytes), nil
}
...
func buildXPrefixOccupancyIndex(points []Point) map[string]map[int64]struct{} {
	...
	shards := make([]map[string]map[int64]struct{}, workers)
	if err := parallelFor(workers, func(worker int) error {
		start := worker * len(points) / workers
		end := (worker + 1) * len(points) / workers
		shards[worker] = buildXPrefixOccupancyIndexSerial(points[start:end])
		return nil
	}); err != nil {
		panic(err)
	}
...
func buildXPrefixOccupancyIndexSerial(points []Point) map[string]map[int64]struct{} {
	index := make(map[string]map[int64]struct{})
	for _, point := range points {
...
type xSubtreeTask struct {
	Prefix string
	MinX   int64
	MaxX   int64
}

func globalEmptyPatternsForXSubtree(
	...
	patterns = append(patterns, globalEmptyPatternsForXSubtree(index, prefixX+"0", minX, midX)...)
	patterns = append(patterns, globalEmptyPatternsForXSubtree(index, prefixX+"1", midX+1, maxX)...)
	return selectMaximalPatterns(patterns)
}
...
func buildGlobalEmptyRegions2D(index map[string]map[int64]struct{}) []GlobalEmptyRegion {
	const splitDepth = 6
	maxDomain := int64(1<<BitLength) - 1
	ancestorPatterns := make([]string, 0)
	tasks := make([]xSubtreeTask, 0, 1<<splitDepth)

	var buildFrontier func(prefixX string, minX, maxX int64)
	buildFrontier = func(prefixX string, minX, maxX int64) {
		occupied := index[prefixX]
		if len(occupied) == 0 {
			ancestorPatterns = append(ancestorPatterns,
				FormatToWildcardPattern(join2DPrefix(prefixX, ""), NumDims, BitLength),
			)
			return
		}
		if len(prefixX) == splitDepth || len(prefixX) == BitLength {
			tasks = append(tasks, xSubtreeTask{Prefix: prefixX, MinX: minX, MaxX: maxX})
			return
		}

		for _, prefixY := range emptyYCoversForBounds(0, maxDomain, occupied) {
			ancestorPatterns = append(ancestorPatterns, FormatToWildcardPattern(join2DPrefix(prefixX, prefixY), NumDims, BitLength))
		}

		midX := minX + (maxX-minX)/2
		buildFrontier(prefixX+"0", minX, midX)
		buildFrontier(prefixX+"1", midX+1, maxX)
	}
	buildFrontier("", 0, maxDomain)

	localPatterns := make([][]string, len(tasks))
	if err := parallelFor(len(tasks), func(idx int) error {
		task := tasks[idx]
		localPatterns[idx] = globalEmptyPatternsForXSubtree(index, task.Prefix, task.MinX, task.MaxX)
		return nil
	}); err != nil {
		panic(err)
	}

	patterns := ancestorPatterns
...
func deriveGlobalEmptyParentKeys(
	...
	orderedKeys := make([]DerivedPatternKey, len(regions))
...
func queryScopedEmptyPatternsFromGlobal(global []GlobalEmptyRegion, query RangeQuery) ([]string, error) {
	...
	if err := parallelFor(len(global), func(idx int) error {
		region := global[idx]
...
func deriveQueryScopedEmptyKeysFromOfflineParents(
	...
	if err := parallelFor(len(scopedPatterns), func(idx int) error {
		parentPattern := parentPatterns[idx]
		parentKey, ok := globalParentKeys[parentPattern]
		if !ok {
			return fmt.Errorf("offline parent key missing for global empty pattern %s", parentPattern)
		}

		childKey, err := derivePatternKeyFromParent(params, parentKey, scopedPatterns[idx], nil)
		if err != nil {
			return err
		}
		scopedKeys[idx] = childKey
		return nil
	}); err != nil {
		return nil, 0, err
	}
...
func deriveInitialPatternKeys(
	...
	if err := parallelFor(len(initialPatterns), func(idx int) error {
		derived, err := derivePatternKeyFromRoot(params, msk, initialPatterns[idx], bitOrder)
		if err != nil {
			return err
		}
		keys[idx] = derived
...
func deriveEmptyPatternKeys(
	...
	if err := parallelFor(len(emptyPatterns), func(idx int) error {
		pattern := emptyPatterns[idx]
		parentIdx := -1
		for candidateIdx, initialKey := range initialKeys {
...
func verifyAllDerivedPatternKeys(params *wkdibe.Params, derived []DerivedPatternKey) (int, error) {
	if err := parallelFor(len(derived), func(idx int) error {
		item := derived[idx]
		message := new(cryptutils.Encryptable).Random()
		ciphertext := wkdibe.Encrypt(message, params, item.Attrs)
		decrypted := wkdibe.Decrypt(ciphertext, item.Key)
...
func verifyGlobalEmptyRegions2D(
	...
	workers := effectiveWorkers(len(regions))
	localCoverBits := make([][]uint64, workers)
	if err := parallelFor(workers, func(worker int) error {
		coverage := make([]uint64, len(realBits))
		start := worker * len(regions) / workers
		end := (worker + 1) * len(regions) / workers
		for regionIdx := start; regionIdx < end; regionIdx++ {
			bounds, err := patternToBounds(regions[regionIdx].Pattern)
			if err != nil {
				return fmt.Errorf("global region %d malformed: %w", regionIdx, err)
			}
			for x := int(bounds[0][0]); x <= int(bounds[0][1]); x++ {
				...
				for y := int(bounds[1][0]); y <= int(bounds[1][1]); y++ {
					...
					if realBits[idx/64]&mask != 0 {
						return fmt.Errorf("global empty region %d covers real point [%d %d]", regionIdx, x, y)
					}
					coverage[idx/64] |= mask
				}
			}
		}
		localCoverBits[worker] = coverage
		return nil
	}); err != nil {
		return 0, 0, err
	}
	...
	if err := parallelFor(len(realBits), func(wordIdx int) error {
		var emptyWord uint64
		for worker := 0; worker < workers; worker++ {
			emptyWord |= localCoverBits[worker][wordIdx]
		}
		if realBits[wordIdx]&emptyWord != 0 {
			return fmt.Errorf("global empty cover intersects real data in bitset word %d", wordIdx)
		}
		if realBits[wordIdx]|emptyWord != ^uint64(0) {
			return fmt.Errorf("global empty cover leaves an uncovered domain point in bitset word %d", wordIdx)
		}
		atomic.AddInt64(&uniqueRealPoints, int64(bits.OnesCount64(realBits[wordIdx])))
		atomic.AddInt64(&coveredEmptyPoints, int64(bits.OnesCount64(emptyWord)))
		return nil
	}); err != nil {
		return 0, 0, err
	}
...
func queryPointIndex(query RangeQuery, p Point) (int, bool) {
	...
	for dim := 0; dim < NumDims; dim++ {
		minValue := query.Bounds[dim][0]
		maxValue := query.Bounds[dim][1]
		if p.Coords[dim] < minValue || p.Coords[dim] > maxValue {
			return 0, false
		}
		width := int(maxValue - minValue + 1)
		idx = idx*width + int(p.Coords[dim]-minValue)
	}
	return idx, true
}
...
func verifyEmptyRegionPatterns(
	...
	totalVolume := 1
	for dim := 0; dim < NumDims; dim++ {
		width := query.Bounds[dim][1] - query.Bounds[dim][0] + 1
		if width <= 0 {
			return 0, 0, fmt.Errorf("invalid query bounds at dimension %d", dim)
		}
		totalVolume *= int(width)
	}
	coverage := make([]int32, totalVolume)
...
func main() {
	...
	workers := flag.Int("mhibe-workers", runtime.NumCPU(), "number of workers for offline initialization, online proving, and experimental audits")
	flag.Parse()
	...
	fmt.Println("[*] Mode: 2D Query-Independent Offline X-Parent Supplement + Full Parallel Protocol + Full Cryptographic Audit")
	...
	var globalCoverAuditMs float64
	if *verifyOfflineGlobal {
		...
		globalCoverAuditMs = float64(time.Since(globalAuditStart).Nanoseconds()) / 1e6
		if auditErr != nil {
	...
	fmt.Printf("4. Cover Strategy: query-independent offline global parents + full parallel online intersection/delegation\n")
	...
	fmt.Printf("Architecture: 2D Query-Independent Offline M-HIBE X-Parent Supplement + Full Parallel Protocol + Separate Full Audits + ZK-Accumulator\n")
