...
func loadTPCPartition(dataPath string, limit int, discountScale int64, query RangeQuery) (loadedPartition, error) {
	...
	for scanner.Scan() {
		...
		qFloat, err := strconv.ParseFloat(cols[4], 64)
		if err != nil {
			return loadedPartition{}, fmt.Errorf("parse l_quantity in row %d: %w", part.rows+1, err)
		}
...
func padNonMembersToPowerOfTwo(dbSet, emptySet []mcl.Fr) []mcl.Fr {
	...
	for nonce := 0; len(padded) < target; nonce++ {
		fr := bpacc.SeedToFr(fmt.Sprintf("__zkacc_pad_3d_%d", nonce))
		key := frSetKey(fr)
		if _, ok := seen[key]; ok {
			continue
		}
...
func main() {
	...
	poneglyphQ6 := flag.Bool("poneglyph-q6", false, "use PoneglyphDB q6_final_v2 bounds: 1994-01-01 < shipdate < 1995-01-01, 0.05 < discount < 0.07, quantity < 24")
	...
	fmt.Println("[*] Mode: 3D full correctness + full completeness")
	...
	fmt.Printf("[*] Query matched %d rows, collapsed into %d unique 3D points.\n",
	...
	fmt.Println("Architecture: 3D Native ZK-Accumulator Full Correctness + Full Completeness Baseline")
