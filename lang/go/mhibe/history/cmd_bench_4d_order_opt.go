...
func pointInBaseQuery(p Point, dateMin, dateMax, discountMin, discountMax, taxMin, taxMax, quantityMin, quantityMax int64) bool {
	...
	if NumDims > 2 && (p.Coords[2] < taxMin || p.Coords[2] > taxMax) {
		return false
	}
	if NumDims > 3 && (p.Coords[3] < quantityMin || p.Coords[3] > quantityMax) {
		return false
	}
	return true
}

func buildDefaultQuery(points []Point, dateMin, dateMax, discountMin, discountMax, taxMin, taxMax, quantityMin, quantityMax, extraWidth int64) RangeQuery {
	...
	if NumDims > 2 {
		query.Bounds[2] = [2]int64{taxMin, taxMax}
	}
	if NumDims > 3 {
		query.Bounds[3] = [2]int64{quantityMin, quantityMax}
	}

	var exemplar *Point
	for i := range points {
		if pointInBaseQuery(points[i], dateMin, dateMax, discountMin, discountMax, taxMin, taxMax, quantityMin, quantityMax) {
			exemplar = &points[i]
			break
		}
	}
...
func printQueryBounds(query RangeQuery) {
	labels := [NumDims]string{"date", "discount", "tax", "quantity"}
	for d := 0; d < NumDims; d++ {
		fmt.Printf("    dim%d %-8s [%d, %d]\n", d, labels[d], query.Bounds[d][0], query.Bounds[d][1])
	}
}
...
func main() {
	...
	fmt.Println("[*] Mode: 4D Dimension-Order Optimized Cover (date->discount->tax->quantity) + Parallel M-HIBE + ZK-Accumulator (Authenticity)")
	...
	fmt.Printf("Architecture: 4D Dimension-Order Optimized Parallel M-HIBE Cover + ZK-Accumulator (Lightweight Auth)\n")
