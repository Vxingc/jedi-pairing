...
func newEmptyIndex3D() EmptyIndex3D {
	return EmptyIndex3D{
		OccupiedX: make(map[int64]struct{}),
		XToY:      make(map[string]map[int64]struct{}),
		XYToZ:     make(map[string]map[int64]struct{}),
	}
}

func build3DPrefixOccupancyIndexSerial(points []Point) EmptyIndex3D {
	index := newEmptyIndex3D()

	for _, point := range points {
		index.OccupiedX[point.Coords[0]] = struct{}{}
...
func mergeOccupiedValues(dst, src map[string]map[int64]struct{}) {
	for key, srcValues := range src {
		dstValues := dst[key]
		if dstValues == nil {
			dstValues = make(map[int64]struct{}, len(srcValues))
			dst[key] = dstValues
		}
		for value := range srcValues {
			dstValues[value] = struct{}{}
		}
	}
}
...
func build3DPrefixOccupancyIndex(points []Point) EmptyIndex3D {
	...
	if workers == 1 {
		return build3DPrefixOccupancyIndexSerial(points)
	}

	shards := make([]EmptyIndex3D, workers)
	if err := parallelFor(workers, func(worker int) error {
		start := worker * len(points) / workers
		end := (worker + 1) * len(points) / workers
		shards[worker] = build3DPrefixOccupancyIndexSerial(points[start:end])
		return nil
	}); err != nil {
		panic(err)
	}
	...
	for _, shard := range shards {
		for x := range shard.OccupiedX {
			index.OccupiedX[x] = struct{}{}
		}
		mergeOccupiedValues(index.XToY, shard.XToY)
		mergeOccupiedValues(index.XYToZ, shard.XYToZ)
	}
	return index
}
...
func occupiedPrefixes(values map[int64]struct{}) []string {
	seen := make(map[string]struct{}, len(values)*(BitLength+1))
	for value := range values {
		valueBin := fmt.Sprintf("%0*b", BitLength, value)
		for prefixLen := 0; prefixLen <= BitLength; prefixLen++ {
			seen[valueBin[:prefixLen]] = struct{}{}
		}
	}
...
func buildDatabaseWideGlobalEmptyRegions3D(index EmptyIndex3D) []GlobalEmptyRegion {
	maxDomain := int64(math.Pow(2, BitLength)) - 1
	patterns := make([]string, 0)

	// Empty X subtrees cover all Y and Z values and need no deeper traversal.
	for _, prefixX := range emptyCoversForBounds(0, maxDomain, index.OccupiedX) {
		patterns = append(patterns, FormatToWildcardPattern(prefixX+"||||", NumDims, BitLength))
	}

	xPrefixes := make([]string, 0, len(index.XToY))
	for prefixX := range index.XToY {
		xPrefixes = append(xPrefixes, prefixX)
	}
	sort.Slice(xPrefixes, func(i, j int) bool {
		if len(xPrefixes[i]) != len(xPrefixes[j]) {
			return len(xPrefixes[i]) < len(xPrefixes[j])
		}
		return xPrefixes[i] < xPrefixes[j]
	})

	patternsByX := make([][]string, len(xPrefixes))
	if err := parallelFor(len(xPrefixes), func(idx int) error {
		prefixX := xPrefixes[idx]
		occupiedY := index.XToY[prefixX]
		local := make([]string, 0)

		for _, prefixY := range emptyCoversForBounds(0, maxDomain, occupiedY) {
			prefix := prefixX + "||" + prefixY + "||"
			local = append(local, FormatToWildcardPattern(prefix, NumDims, BitLength))
		}

		// XYToZ already contains every occupied Y prefix for this X prefix.
		// Iterating those prefixes replaces a full 8191-node Y-tree scan.
		for _, prefixY := range occupiedPrefixes(occupiedY) {
			occupiedZ := index.XYToZ[prefixPairKey(prefixX, prefixY)]
			if len(occupiedZ) == 0 {
				continue
			}
			for _, prefixZ := range emptyCoversForBounds(0, maxDomain, occupiedZ) {
				prefix := prefixX + "||" + prefixY + "||" + prefixZ
				local = append(local, FormatToWildcardPattern(prefix, NumDims, BitLength))
			}
		}

		patternsByX[idx] = selectMaximalPatterns(local)
		return nil
	}); err != nil {
		panic(err)
	}

	for _, local := range patternsByX {
...
func deriveGlobalEmptyParentKeysOffline(
	...
	progressStep := (len(regions) + 19) / 20
	var completed int64
	if err := parallelFor(len(regions), func(idx int) error {
		key, err := derivePatternKeyFromRoot(params, msk, regions[idx].Pattern, nil)
		if err != nil {
			return fmt.Errorf("global empty parent %d: %w", idx, err)
		}
		ordered[idx] = key
		done := atomic.AddInt64(&completed, 1)
		if len(regions) >= 1000 && (done%int64(progressStep) == 0 || done == int64(len(regions))) {
			fmt.Printf("        [Progress] Parent keys: %d/%d (%.0f%%)\n", done, len(regions), 100*float64(done)/float64(len(regions)))
		}
		return nil
	}); err != nil {
		return nil, 0, err
...
func deriveQueryScopedKeysFromOfflineParents(
	...
	parentPatterns := make([]string, len(scopedPatterns))
	if err := parallelFor(len(scopedPatterns), func(idx int) error {
		for _, region := range global {
			if patternContainsPattern(region.Pattern, scopedPatterns[idx]) {
				parentPatterns[idx] = region.Pattern
				return nil
			}
		}
		return fmt.Errorf("no offline parent contains query-scoped pattern %s", scopedPatterns[idx])
	}); err != nil {
		return nil, 0, err
	}
	keys := make([]DerivedPatternKey, len(scopedPatterns))
	if err := parallelFor(len(scopedPatterns), func(idx int) error {
		parentKey, ok := parentKeys[parentPatterns[idx]]
		if !ok {
			return fmt.Errorf("offline parent key missing for %s", parentPatterns[idx])
		}
		key, err := derivePatternKeyFromParent(params, parentKey, scopedPatterns[idx], nil)
		if err != nil {
			return err
		}
		keys[idx] = key
...
func canonicalPatternPrefixes(pattern string) ([NumDims]string, bool) {
	var prefixes [NumDims]string
	if len(pattern) != NumDims*BitLength {
		return prefixes, false
	}

	for dim := 0; dim < NumDims; dim++ {
		segment := pattern[dim*BitLength : (dim+1)*BitLength]
		prefixLen := 0
		seenWildcard := false
		...
		prefixes[dim] = segment[:prefixLen]
	}
	return prefixes, true
}

func selectedAncestorExists(selected map[[NumDims]string]struct{}, candidate [NumDims]string) bool {
	var ancestor [NumDims]string
	for xLen := 0; xLen <= len(candidate[0]); xLen++ {
		ancestor[0] = candidate[0][:xLen]
		for yLen := 0; yLen <= len(candidate[1]); yLen++ {
			ancestor[1] = candidate[1][:yLen]
			for zLen := 0; zLen <= len(candidate[2]); zLen++ {
				ancestor[2] = candidate[2][:zLen]
...
func selectMaximalPatterns(patterns []string) []string {
	...
	selectedPrefixes := make(map[[NumDims]string]struct{}, len(unique))
	nonCanonical := make([]string, 0)
	for _, candidate := range unique {
		candidatePrefixes, canonical := canonicalPatternPrefixes(candidate)
		dominated := canonical && selectedAncestorExists(selectedPrefixes, candidatePrefixes)
		if !dominated {
			chosenPatterns := nonCanonical
			if !canonical {
				chosenPatterns = maximal
			}
			for _, chosen := range chosenPatterns {
		...
		if !dominated {
			...
			if canonical {
				selectedPrefixes[candidatePrefixes] = struct{}{}
			} else {
				nonCanonical = append(nonCanonical, candidate)
			}
		}
	}
...
func main() {
	...
	fmt.Println("[*] Mode: 3D Query-Independent Occupied-Prefix Offline Cover + Full Parallel Protocol")
	...
	fmt.Printf("    [Trace] Starting parallel database prefix-index construction with %d workers...\n", mhibeWorkers)
	...
	fmt.Printf("    [Trace] Parallel database prefix-index construction: %.2f ms\n", emptyIndexMs)
	fmt.Println("    [Trace] Starting occupied-prefix global empty-region generation...")
	globalRegionStart := time.Now()
	globalEmptyRegions := buildDatabaseWideGlobalEmptyRegions3D(databaseEmptyIndex)
	globalRegionMs := float64(time.Since(globalRegionStart).Nanoseconds()) / 1e6
	fmt.Printf("    [Trace] Occupied-prefix global empty-region generation: %.2f ms (%d regions)\n", globalRegionMs, len(globalEmptyRegions))
	fmt.Println("    [Trace] Starting parallel database-wide parent-key generation...")
	...
	fmt.Printf("    [Trace] Parallel database-wide parent-key generation: %.2f ms\n", globalParentKeyMs)
	...
	fmt.Printf("4. Cover Strategy: parallel occupied-prefix offline extraction + parallel online intersection/delegation\n")
	...
	fmt.Printf("Architecture: 3D Query-Independent Occupied-Prefix M-HIBE + Full Parallel Protocol + ZK-Accumulator\n")
	totalSetupMs := setupMs + offlineInitMs + zkCommitMs
	totalServerMs := engineAMs + zkProverMs
	totalClientMs := geometryMs + zkVerifierMs
	...
	fmt.Printf("Total Protocol Time: %.2f ms\n", totalSetupMs+totalServerMs+totalClientMs)
}
