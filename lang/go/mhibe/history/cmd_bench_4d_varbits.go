...
func dimBitLength(dim int) int {
	return DimBitLengths[dim]
}

func totalBitLength() int {
	total := 0
	for _, bitLen := range DimBitLengths {
		total += bitLen
	}
	return total
}

func dimMaxDomain(dim int) int64 {
	return (int64(1) << dimBitLength(dim)) - 1
}

func clampToDimDomain(value int64, dim int) int64 {
	if value < 0 {
		return 0
	}
	maxDomain := dimMaxDomain(dim)
	if value > maxDomain {
		return maxDomain
	}
	return value
...
func MapToIDs(query RangeQuery) []string {
	...
	for i := 0; i < NumDims; i++ {
		dimCovers = append(dimCovers, getCanonicalCover(query.Bounds[i][0], query.Bounds[i][1], 0, dimMaxDomain(i), ""))
	}
...
func FormatToWildcardPattern(prefix string, numDims int, bitLen int) string {
	...
	for d := 0; d < numDims; d++ {
		...
		for i := len(dims[d]); i < dimBitLength(d); i++ {
			b.WriteByte('*')
		}
	}
	return b.String()
...
func FormatPointToBinary(p Point) string {
	var b strings.Builder
	for i := 0; i < NumDims; i++ {
		b.WriteString(fmt.Sprintf("%0*b", dimBitLength(i), clampToDimDomain(p.Coords[i], i)))
	}
	return b.String()
}
...
func buildNDPrefixOccupancyIndex(points []Point, query RangeQuery, maxParentDim int) EmptyIndexND {
	...
	for _, point := range points {
		...
		for d := 0; d < NumDims; d++ {
			prefixOptions[d] = prefixesForValueIntersectingQuery(point.Coords[d], query.Bounds[d][0], query.Bounds[d][1], dimBitLength(d))
		}
...
func emptyCoversForBounds(minVal, maxVal int64, occupied map[int64]struct{}, dim int) []string {
...
func collectIntersectingPrefixNodes(minQuery, maxQuery int64, dim int) []PrefixNode {
	maxDomain := dimMaxDomain(dim)
	...
	walk = func(prefix string, minVal, maxVal int64) {
		...
		if len(prefix) == dimBitLength(dim) {
			return
		}
...
func buildQueryTouchedGlobalEmptyRegionsND(
	...
	for d := 0; d < NumDims; d++ {
		nodesByDim[d] = collectIntersectingPrefixNodes(query.Bounds[d][0], query.Bounds[d][1], d)
	}
	...
	recurse = func(nextDim int, parentPrefixes []string) {
		if stop || nextDim >= NumDims {
			return
		}

		occupied := index.occupiedValues(parentPrefixes, nextDim)
		for _, emptyPrefix := range emptyCoversForBounds(query.Bounds[nextDim][0], query.Bounds[nextDim][1], occupied, nextDim) {
	...
	for _, firstNode := range nodesByDim[0] {
	...
	if stop {
		panic(fmt.Sprintf("global empty region generation exceeded -max-global-regions=%d; narrow the query, lower -limit, or raise the cap", maxGlobalRegions))
	}
...
func prefixesFromBounds(minVal, maxVal int64, dim int) []string {
	return getCanonicalCover(minVal, maxVal, 0, dimMaxDomain(dim), "")
}
...
func intersectPatternWithQuery(pattern string, query RangeQuery) ([]string, bool, error) {
	...
	for d := 0; d < NumDims; d++ {
		...
		dimCovers = append(dimCovers, prefixesFromBounds(minVal, maxVal, d))
	}
...
func buildDefaultQuery(points []Point, dateMin, dateMax, discountMin, discountMax, quantityMin, quantityMax, extraWidth int64) RangeQuery {
	...
	for d := 0; d < NumDims; d++ {
		query.Bounds[d] = [2]int64{0, dimMaxDomain(d)}
	}
...
func printQueryBounds(query RangeQuery) {
	labels := [NumDims]string{"date", "discount", "quantity", "tax"}
	for d := 0; d < NumDims; d++ {
		fmt.Printf("    dim%d %-8s [%d, %d] (%d-bit)\n", d, labels[d], query.Bounds[d][0], query.Bounds[d][1], dimBitLength(d))
	}
}
...
func main() {
	...
	maxGlobalRegions := flag.Int("max-global-regions", 200000, "abort if query-touched global empty region generation exceeds this cap; 0 disables the cap")
	...
	fmt.Println("[*] Mode: 4D Variable-Bit-Length Cover (12,4,6,4) + Parallel M-HIBE + ZK-Accumulator (Authenticity)")
	...
	databaseEmptyIndex := buildNDPrefixOccupancyIndex(dbData, query, *maxParentDim)
	emptyIndexMs := float64(time.Since(emptyIndexStart).Nanoseconds()) / 1e6
	fmt.Printf("[+] Database Empty Index Build Time: %.2f ms\n", emptyIndexMs)
	fmt.Printf("[+] Empty Index Mode: precomputed query-touched fixed-order parent occupancy\n")
	fmt.Printf("[+] Indexed Query-Touched Parent Nodes: %d\n", totalIndexedParentNodes(databaseEmptyIndex))
	...
	fmt.Println("    [Trace] Starting global empty parent generation...")
	stepStart := time.Now()
	globalEmptyRegions := buildQueryTouchedGlobalEmptyRegionsND(query, &databaseEmptyIndex, *maxGlobalRegions, *maxParentDim)
	fmt.Printf("    [Trace] Global empty parent generation: %.2f ms (%d regions)\n", float64(time.Since(stepStart).Nanoseconds())/1e6, len(globalEmptyRegions))
	...
	fmt.Printf("4. Cover Strategy: offline fixed-order parent empty regions + online query intersection/parallel delegation\n")
	...
	fmt.Printf("Architecture: 4D Variable-Bit-Length Parallel M-HIBE Cover + ZK-Accumulator (Lightweight Auth)\n")
