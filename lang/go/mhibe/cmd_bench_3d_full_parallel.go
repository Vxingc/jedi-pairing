...
func build3DPrefixOccupancyIndex(points []Point) EmptyIndex3D {
	...
	for _, shard := range shards {
		mergeOccupiedIndex(index.XToY, shard.XToY)
		mergeOccupiedIndex(index.XYToZ, shard.XYToZ)
	}
	return index
}
...
func buildQueryTouchedGlobalEmptyRegions3D(
	...
	localPatterns := make([][]string, len(xNodes))
	if err := parallelFor(len(xNodes), func(i int) error {
		xNode := xNodes[i]
...
func bestCoverCandidateParallel(candidates []CoverCandidate, uncovered []uint64) (int, int) {
	workers := effectiveWorkers(len(candidates))
	...
	if err := parallelFor(workers, func(worker int) error {
		...
		for idx := start; idx < end; idx++ {
			gain := intersectionCount(candidates[idx].Coverage, uncovered)
			if gain > bestGain || (gain == bestGain && gain > 0 && (bestIdx == -1 || idx < bestIdx)) {
				bestIdx = idx
				bestGain = gain
			}
		}
		bests[worker] = localBest{idx: bestIdx, gain: bestGain}
		return nil
	}); err != nil {
		panic(err)
	}
	...
	bestIdx := -1
	bestGain := 0
	for _, candidate := range bests {
		if candidate.gain > bestGain || (candidate.gain == bestGain && candidate.gain > 0 && (bestIdx == -1 || candidate.idx < bestIdx)) {
...
func greedySetCover(candidates []CoverCandidate, emptyPointCount int) ([]CoverCandidate, error) {
	...
	doneRound := make(chan struct{}, workers)
...
func main() {
	...
	fmt.Println("[*] Mode: 3D XY-Parent Supplement Cover + Full Parallel Extraction/M-HIBE + ZK-Accumulator (Authenticity)")
	...
	globalEmptyRegions := buildQueryTouchedGlobalEmptyRegions3D(query, databaseEmptyIndex)
	...
	fmt.Printf("4. Cover Strategy: parallel global empty intersection + parallel greedy cover + parallel delegation\n")
	...
	fmt.Printf("Architecture: 3D Full-Parallel M-HIBE XY-Parent Supplement Cover + ZK-Accumulator (Lightweight Auth)\n")
