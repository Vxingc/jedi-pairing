...
// cmd_bench_4d_memopt.go — memory-optimised fork of cmd_bench_4d_full_audit_parallel.go
//
// Changes vs original (no semantic difference in results):
//   D1 – early release: databaseEmptyIndex freed after enumeration print; shard maps
//        nilled one-by-one during merge; dbData released after upload stage; dbFr/I/X
//        skipped entirely when -skip-zk is set.
//   D2 – compact EmptyIndexND: inner map[int64]struct{} → sorted []uint16 (domain
//        0..4095 fits in uint16, ~30–50× less overhead per bucket); DerivedPatternKey.Attrs
//        nilled after key derivation so the *big.Int map is not kept alive.
...
type DerivedPatternKey struct {
	Pattern string
	Attrs   wkdibe.AttributeList // nilled after derivation to free *big.Int map
...
// -----------------------------------------------------------------------
// Key derivation
// D2: .Attrs is nilled after key derivation — NonDelegableQualifyKey only
// needs parent.Key and the child's attribute list; the parent's Attrs map
// (holding one *big.Int per fixed bit) is not needed afterwards.
// -----------------------------------------------------------------------
...
func derivePatternKeyFromRoot(
	...
	return DerivedPatternKey{
		Pattern: pattern,
		Attrs:   nil, // D2: don't retain Attrs; child derivation rebuilds its own
...
func derivePatternKeyFromParent(
	...
	if parent.Pattern == childPattern {
		return DerivedPatternKey{Pattern: childPattern, Key: parent.Key}, nil
	}
	...
	return DerivedPatternKey{
		...
		Attrs:   nil, // D2: child doesn't need to retain its Attrs either
...
// -----------------------------------------------------------------------
// Pattern utilities (unchanged from original)
// -----------------------------------------------------------------------
...
func buildCoverCandidates(
	...
	if err := parallelFor(len(patterns), func(idx int) error {
		...
		if !isBitsetZero(coverage) {
			candidateSlots[idx] = CoverCandidate{Pattern: pattern, Coverage: coverage}
			keep[idx] = true
		}
		return nil
	}); err != nil {
...
func (h *coverMaxHeap) Push(x interface{}) { *h = append(*h, x.(coverHeapItem)) }
...
// -----------------------------------------------------------------------
// Data parsing (unchanged from original)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// main — identical flow to the original, with D1 early-release annotations.
// -----------------------------------------------------------------------
...
func main() {
	...
	poneglyphQ6 := flag.Bool("poneglyph-q6", false, "use PoneglyphDB-compatible 3D range bounds")
	dateMin := flag.String("date-min", "1994-01-01", "inclusive shipdate lower bound")
	dateMax := flag.String("date-max", "1994-12-31", "inclusive shipdate upper bound")
	discountScale := flag.Int64("discount-scale", 100, "scale factor for encoding l_discount")
	...
	taxMin := flag.Int64("tax-min", 2, "inclusive encoded l_tax lower bound for dim3")
	taxMax := flag.Int64("tax-max", 6, "inclusive encoded l_tax upper bound for dim3")
	...
	fmt.Println("[*] Starting MEMOPT Benchmark (D1+D2 memory optimisations)...")
	...
	// D1: build dbFr/I/X only when ZK is actually needed.
	var dbFr []mcl.Fr
	var I []mcl.Fr
	var X []mcl.Fr
	queryUniquePoints := make(map[[NumDims]int64]struct{})
	var queryMatchRows int
	for _, p := range dbData {
		if IsPointInQuery(p, query) {
			queryUniquePoints[p.Coords] = struct{}{}
			queryMatchRows++
			if !*skipZK {
				I = append(I, bpacc.SeedToFr(FormatPointToBinary(p)))
			}
		} else {
			if !*skipZK {
				X = append(X, bpacc.SeedToFr(FormatPointToBinary(p)))
			}
		}
	}
	if !*skipZK {
		dbFr = make([]mcl.Fr, 0, len(dbData))
		// Reconstruct dbFr in original order for acc.Commit(dbFr).
		for _, p := range dbData {
			dbFr = append(dbFr, bpacc.SeedToFr(FormatPointToBinary(p)))
		}
	}
	...
	fmt.Printf("[*] Query matched %d unique spatial points (from %d rows).\n", len(queryUniquePoints), len(I))

	// 2. Offline initialisation
	...
	// D1: dbData is no longer needed after the index is built (and the optional
	// upload stage below). We keep the slice alive until after the upload stage,
	// then nil it explicitly so the GC can reclaim the []Point backing array.
	...
	offlineMaterial := OfflineEmptyMaterial{
		Regions:        globalEmptyRegions,
		ParentKeys:     globalParentKeys,
		ParentKeyBytes: globalParentKeyBytes,
	}
	...
	// D1: dbData no longer needed after upload stage.
	dbData = nil
	runtime.GC()

	// 3. Engine A
	fmt.Println("\n=== ENGINE A: 4D M-HIBE FIXED-ORDER PARENT SUPPLEMENT ===")
	extractionStart := time.Now()

	initialPrefixes := MapToIDs(query)
	initialPatterns := make([]string, 0, len(initialPrefixes))
	...
	// D1: bitset candidates no longer needed after greedy selection.
	coverCandidates = nil
	...
	verificationEmptyKeys, touchedGlobalParentKeyBytes, err := deriveQueryScopedKeysFromOfflineParents(
		params, offlineMaterial.Regions, offlineMaterial.ParentKeys, selectedCoverPatterns)
	if err != nil {
		panic(err)
	}
	...
	fmt.Printf("2. Database-Wide Global Empty Parent Regions: %d\n", len(offlineMaterial.Regions))
	...
	if verifyErr == nil && coveredEmptyPoints+realSpatialVolume == totalQueryVolume {
		fmt.Printf("[+] Client Protocol Geometric Completeness Time: %.4f ms (SUCCESS!)\n", geometryMs)
		fmt.Printf("    -> [Detail] %d unique real spatial points.\n", realSpatialVolume)
		fmt.Printf("    -> [Detail] %d empty-region keys cover %d empty spatial points.\n", len(verificationEmptyKeys), coveredEmptyPoints)
		fmt.Printf("    -> [Detail] Maximum overlap multiplicity: %d.\n", maxOverlap)
	} else {
		if verifyErr != nil {
	...
	fmt.Printf("Memory optimisations: D1 (early release of index/dbData/shards/bitsets) + D2 (compact uint16 index, no retained Attrs)\n")
